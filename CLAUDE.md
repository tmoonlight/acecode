...
`agent_loop.question_policy`(`ask` 默认 / `deny` / `timeout`,CLI `--question-policy` 覆盖只写运行时字段不落盘)控制 AskUserQuestion 应答:deny 不弹 UI 直接返回「自行决策并继续」自动应答;timeout 等 `question_timeout_seconds`(默认 60,[5,3600])秒后自动采纳每题第一选项(output/metadata 标注自动采纳)。优先级:goal 无人值守 > 显式配置 > YOLO 隐式 deny > 默认 ask。解析纯函数 `src/tool/question_policy.{hpp,cpp}`,经 `ToolContext::question_policy` 探针注入 —— deny/ask 分支实时反映权限模式(`/yolo` 切换即生效),daemon 的 timeout 窗口在 session 创建时固化到 `AskUserQuestionPrompter`(已知限制)。见 openspec/changes/add-ask-question-policy。
