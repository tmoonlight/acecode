...
The smooth transcript path is `acecode::tui::ChatViewport` in `src/tui/chat_viewport.*`, and it is enabled by default. Set `ACECODE_TUI_CHAT_VIEWPORT=0` only when comparing or rolling back to the legacy `vbox(message_elements) | focusPosition | yframe` path. It is an ordinary FTXUI custom component (`ComponentBase` + custom `Node`) that renders only visible rows to the provided `ftxui::Screen`; it does not write stdout, ANSI, or run a separate terminal loop. `ACECODE_TUI_CHAT_VIEWPORT_TRACE=1` adds debug log rows with visible row range, chat/scrollbar boxes, and cache hit/miss/build counters for scroll-performance checks. `ACECODE_TUI_SCROLL_TRACE=1` logs wheel routing and row deltas in `main.cpp`, including the legacy rollback path. `ACECODE_TUI_FORCE_CONHOST_COMPAT_LAYOUT=1` is a diagnostic-only override for smoke testing the ConHost compatibility layout branch. `ACECODE_TUI_FORCE_OSC52_COPY=1` is a diagnostic-only override for right-click copy validation that skips system clipboard writes and emits OSC 52 directly. Use `.\scripts\tui_chat_viewport_validation.ps1 -Mode PrintCommands` for the recommended Windows validation flow. The wrapper can preflight the current shell with `-Mode CheckHost -TerminalHost windows-terminal|classic-conhost`, runs preflight checks with `-Mode Preflight`, runs `scripts\tui_chat_viewport_manual.py --coverage-matrix` per terminal host, aggregates directories with `scripts\tui_chat_viewport_report.py --strict`, can add `--update-tasks openspec\changes\add-ftxui-chat-viewport\tasks.md --write-evidence openspec\changes\add-ftxui-chat-viewport\manual-validation-evidence.md`, and runs `scripts\tui_chat_viewport_cleanup_check.py --strict` after 10.1/10.2 cleanup. The underlying report script accepts report files or directories expanded as `manual-report*.json`.
