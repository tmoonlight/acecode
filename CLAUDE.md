...
**`app_config_mu` 是 shared_mutex,默认独占、resume 走共享。** 54 处既有锁点全部保持 `lock_guard`(独占,与旧 std::mutex 语义一致);仅两条 web resume 路由降级 `shared_lock` —— resume 对 config 只读但要全量解析 jsonl(实测 574~824ms/次),独占持有会把整个 HTTP 面卡停近一秒,用户在侧边栏连点几个会话就是一串锁车队(feedback IQSZ-D0668)。新增读路径想降共享前先确认全程不写 config;写方(settings 变更/saved_models 落盘/refresh_default_session_preferences)必须独占。同 id 并发 resume 由 `SessionRegistry::resume` 入口的单飞守卫(`resume_inflight_`)串行化,后到者等首个完成后命中 entries_ 快速路径 —— 没有这层,两个 make_entry 并发会互抢 writer lease,输家析构时还可能清掉赢家的 lease。
...
**`/api/git/info` 必须走共享缓存 `web/src/lib/gitInfoCache.js`(单例,30s TTL + 在途去重),别再各建一份。** 该端点在 daemon 侧一次要 spawn 5~7 个 git 子进程(`rev-parse` / `symbolic-ref` / `show-ref`×N / `for-each-ref` / `status --porcelain`),Windows 上光进程启动就上百毫秒。三个消费方(GitSessionPill、Sidebar hover 卡片、SidePanel)共用一个 cwd,同 workspace 下命中率极高。**GitSessionPill 还有一条额外短路**:它在 ChatView 上的 key 含 sid,每次切会话整组件重挂,而 bar 变体在「会话已开始 / 已在 worktree」时 `return null` —— 必须用 `pillCouldRender` 在**发请求之前**短路,否则切到任何一个已有消息的会话都白打一次 git.exe ×6(feedback IQSZ-D0668:用户在会话间切换时 Fiddler 里 `/api/git/info` 高频且经常挂起)。注意 `GIT_STATE_CHANGED_EVENT` 目前**只有监听方没有发送方**(pill 的主仓 checkout 能力移除后遗留),重新接回 checkout 时需一并恢复广播。
