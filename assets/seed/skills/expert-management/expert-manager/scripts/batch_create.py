...
def build_agent(package: Path, item: dict[str, Any]) -> None:
...
def build_team(package: Path, item: dict[str, Any]) -> None:
    name = require_text(item, "name")
    display_name = require_text(item, "displayName")
    profession = require_text(item, "profession")
    description = require_text(item, "displayDescription")
...
def create_one(item: dict[str, Any], root: Path) -> tuple[bool, str]:
    ...
    try:
        with tempfile.TemporaryDirectory(
            ...
            if expert_type == "agent":
                build_agent(package, item)
            else:
                build_team(package, item)
...
def main() -> int:
    ...
    for raw in experts:
        if not isinstance(raw, dict):
            ...
            continue
        ok, message = create_one(raw, root)
