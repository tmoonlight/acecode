...
description: Find and position a viable niche market for a one-person company by combining market mapping and customer segmentation. Use when ACECode needs to explain niche concepts when needed, check founder-resource prerequisites, ask one question at a time, generate multiple niche options, and write user-confirmed positioning outputs into `opc-doc/`.
---
...
用户明确确认目标利基和定位陈述后，**立即**使用当前可用的文件写入工具创建以下文件。在对话中描述结论不等于落盘。
