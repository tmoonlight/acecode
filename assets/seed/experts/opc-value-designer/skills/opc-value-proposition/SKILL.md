...
description: Design value propositions for candidate customer segments and help the user choose the strongest one. Use when ACECode needs to explain jobs, pains, and gains when needed, check niche-positioning prerequisites, ask one question at a time, present multiple value-proposition options, and write user-confirmed outputs into `opc-doc/`.
---
...
用户明确确认主价值主张版本后，**立即**使用当前可用的文件写入工具在文件系统上创建以下文件，然后才能进入下一阶段。在对话中描述结论不等于落盘。
