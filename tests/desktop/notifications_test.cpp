...
//
// Windows 固定自绘 layered toast,macOS Desktop 使用 UserNotifications,其他
// 平台保留 no-op 桩。这里覆盖不依赖真实桌面会话的默认值、失败降级与重复清理
// 行为。Payload 解析、Unicode 截断和 activation 路由见
...
TEST(DesktopNotificationsLifecycle, InitWithoutPlatformIdentityIsSafe) {
    ...
    // by the generator.
    // Windows: the self-drawn renderer needs a usable window station, which a
    // headless runner may not have. Either result is valid; lifecycle safety
    // is the contract.
