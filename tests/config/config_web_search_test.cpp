...
//
// 覆盖项:
//   - WebSearchConfig 默认值(enabled=true / backend="rss" / max_results=5 /
    ...
    if (wsj.contains("backend") && wsj["backend"].is_string()) {
        ...
        if (b != "rss" && b != "auto" && b != "duckduckgo" && b != "bing_cn" &&
            ...
            return "web_search.backend='" + b +
                   "' invalid; expected one of: rss, auto, duckduckgo, bing_cn, bochaai, tavily";
        }
...
TEST(ConfigWebSearchDefaults, NestedInAppConfig) {
    ...
    EXPECT_EQ(cfg.web_search.backend, "rss");
...
// 场景:6 个合法 backend 名都被原样接受
TEST(ConfigWebSearchLoader, AcceptsAllValidBackends) {
    for (const std::string& v : {"rss", "auto", "duckduckgo", "bing_cn", "bochaai", "tavily"}) {
        WebSearchConfig c;
