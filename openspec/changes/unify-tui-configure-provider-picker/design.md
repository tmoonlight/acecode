...
现有 `configure_openai()`、`configure_anthropic()`、`configure_copilot()` 以及 catalog Provider 选定后的 Base URL、密钥和模型配置逻辑都已可用。Web 模型目录还在 `model_catalog_handler.cpp` 内单独硬编码 ACEModel 的 ID、固定端点、环境变量和两个内置模型；TUI 只读取 models.dev，因此不会显示 ACEModel。本次重组选择层和 picker 输入方式，并把这组自营 Provider 元数据提升为共享定义，不改变既有认证与持久化语义。

## 目标 / 非目标

**目标：**

- 以一个 FTXUI picker 承载两个自定义接口、ACEModel、Copilot 和全部可运行的 models.dev 预置。
- 让 Web 模型目录与 TUI configure 读取同一份 ACEModel Provider 和内置模型定义。
- 显示始终可见的搜索文本框，直接键入即过滤建议，方向键改变建议项，回车提交当前建议。
- 保持非 TTY stdin 回落路径可脚本化，并允许直接输入查询文本后再按编号选择。
- 根据当前配置高亮对应候选，重新运行 configure 时不把用户带到无关入口。
- catalog 缺失时仍能配置自定义接口、ACEModel 或 Copilot。
...
3. ACEModel；
4. GitHub Copilot；
5. models.dev 中所有 `openai_compat_providers()`。

选择结果使用枚举路由到现有配置函数，避免依赖显示文本或脆弱的数字下标。

替代方案是继续保留四项顶层菜单，只给第二层加搜索框；这不能满足入口合并，也会让用户在搜索 Provider 前多走一步。

### 2. ACEModel 使用共享的内置 catalog 定义

在 `src/provider/` 下提供返回稳定 `ProviderEntry` 的共享 helper，其中集中定义 ACEModel 的 ID、名称、固定 Base URL、`ACEMODEL_API_KEY` 和 `moonlight`、`starrylight` 模型。Web catalog serializer 与 TUI 候选构造都引用该对象；Web 不再维护私有常量和模型数组。

TUI 把共享对象作为 catalog-backed preset 插入第三位，继续复用通用 OpenAI 兼容预置配置流程并保存 `models_dev_provider_id=acemodel`。构造 models.dev 尾部列表时跳过同 ID 条目，保证未来上游加入 ACEModel 后也不会重复。

替代方案是在 `configure_catalog.cpp` 再复制 ACEModel 常量和模型列表；这会让 Web/TUI 的端点或模型更新再次漂移，违反本次同步要求。

### 3. 受管 Copilot 覆盖普通目录项
...
### 5. 拆分“选择目录 Provider”和“配置已选 Provider”
...
- `provider=openai` 且 `models_dev_provider_id=acemodel` 高亮第三项；
- `provider=copilot` 高亮第四项；
...
- **[风险] ACEModel 同时由内置定义和未来 models.dev 提供** → 统一构造按 Provider ID 去重，内置定义优先，确保固定端点和内置模型语义稳定。
