...
- rank 取的是「需要切断的边最少」的全序。关键链是 `llm < … < session < … < provider < tool`:provider→session 的 12 条边、tool→session 的 16 条边按这个顺序本来就合法。原型脚本在 977 个文件、2757 条 include 边上跑出 0 违规。**但这是在草案命名、只检查 rank 与平级规则的条件下得到的**,P0-03 必须用正式的 `layers.tsv`(全部 R 规则)重跑,把实测违规数写进本文。
- 已知的妥协(不作为本期问题处理):
  - include 里看不出分组;
...
5. **热点文件互斥**:同一时刻只允许一个进行中的任务修改以下文件或目录:
   - agent_loop.hpp / agent_loop.cpp(及 P6A 期间 engine/agent 下正在拆的文件);
   - main.cpp(及 P6B 期间 apps/tui/app 下正在拆的文件);
   - CMakeLists.txt;
...
```
P0-01(决策定稿)
 ├→ P0-02 … P0-08(P0-08 → P0-09 串行,其余可并行)
 ├→ P0-10、P0-11、P0-12(随时可做,P0-10 应在 P2-08 搬 agent_loop 之前)
 ├→ P2-01(随时可做,须在 P3 前合入)
 └→ P1(单个 PR,前置:P0-03/04/05/07)
     └→ P2:{02,03,04} 可并行 → 05 → 06 → 07 → 08;09 全程并行
         └→ P3:演练 P3-01 → 冻结搬迁 P3-02 → 分支迁移 P3-03
             ├→ P4
             ├→ P6A:A-01 … A-13 串行 ─┐
             ├→ P6B:B-01 … B-13 串行 ─┼→ A-14(需 A-13 + B-13)→ A-17
             │   (B-11 需 A-01)         ├→ O-10(需 A-14)
             │                          └→ O-05(需 B-13)
             └→ P7-O:O-01、O-06、O-09 随时;O-02 → O-03 → O-04;O-07、O-08 独立;
                     O-11 需 A-14 + O-03
                        └→ 一期验收
```
...
### 8.3 二期待办(各自独立 change,本期只登记,不排期)
