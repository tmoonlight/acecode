...
会话附件上传后已经由 `AttachmentStore` 保存为独立 blob，并在 `AttachmentRecord` 中持久化 `id`、`name`、`mime_type`、`size_bytes` 与 `path`。Desktop 从本机拖入文件时还会把经过服务端校验的绝对 `source_path` 写入 metadata；浏览器上传没有这个来源路径，但仍有会话快照路径。
...
- 同时保留本机原始路径的工作语义与会话快照的持久后备语义。
- 保持附件上传、持久化、恢复、搜索和图片多模态行为不变。

**Non-Goals:**

- 不新增通用文档解析、向量检索或 `attachment_search` 工具。
- 不让 Provider 原生托管 ACECode 的普通文件 blob。
- 不改变附件大小限制、API 请求格式、JSONL schema 或前端拖放交互。
...
没有只保留 `source_path`，因为浏览器上传没有该字段，而且源文件可能在恢复会话前被移动或删除。也没有只保留 `snapshot_path`，因为 Desktop 用户拖入工作区外文件时通常期望 Agent 能定位并操作原文件。
...
无需数据迁移。部署后，新旧会话中的 file part 都在下一次 Provider 序列化时生成引用。回滚只需恢复旧序列化逻辑，附件 blob 与历史 JSONL 仍兼容。
