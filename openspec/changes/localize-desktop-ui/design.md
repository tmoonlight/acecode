...
React's root subscribes through `useTranslation`, and presentation helpers call the initialized application instance at execution time. Memoized leaf components that can render localized helper output subscribe directly as well. For catalogued copy at module scope, the Babel compiler emits lazy object getters and lazy array accessors whose reads call the current `tr(...)` implementation; it also adds the active language to `useMemo` dependencies in localized modules. Direct module-scope translated primitives are rejected by a regression gate and must move behind a function boundary. This keeps module exports and collection identity stable while allowing the same mounted React tree to read the new language immediately.
...
Alternative considered: reloading the WebView after every successful language change. Rejected because it discards transient UI state and is unnecessary once module-scope presentation metadata resolves lazily.
