...
When Settings changes locale, React performs an optimistic `i18n.changeLanguage`, persists through the daemon API, and invokes `aceDesktop_applyLocale` when present. The native bridge updates in-memory locale state used by subsequent tray/dialog/notification presentations. The save and bridge sequence is transactional from the UI's perspective: either both layers accept it, or the daemon preference, native state, and React locale are rolled back. A successful change stores a one-shot locale handoff in `sessionStorage` and schedules a lightweight WebView page reload so module-scope presentation maps are reevaluated without restarting ACECode or its daemon. The handoff takes precedence exactly once over the process-start locale globals injected by the existing WebView init script, then is consumed so later cold navigations continue to use canonical native startup state.
