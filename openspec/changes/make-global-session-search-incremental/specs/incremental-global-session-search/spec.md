...
SearchPalette SHALL 在预热和正文搜索未完成时保持输入、已有结果与关闭操作可用，并 SHALL 使用不占布局高度的细主色进度条显示进度，而不是插入文字状态行或用全局 loading 门遮挡列表。用户取消 MUST 保持安静且不得覆盖最近成功结果；超时或真实错误 MUST 显示可重试状态。目录或正文轮询刷新 MUST 保留用户当前的结果列表滚动位置，且 MUST NOT 因批次到达而重置选择或主动滚动列表。
...
#### Scenario: 搜索进度低调显示且不改变布局
- **WHEN** 目录预热或正文搜索仍在推进
- **THEN** SearchPalette MUST 在搜索输入区底边显示一条细主色进度条
- **AND** 进度提示 MUST NOT 增加状态行、改变结果区高度或显示强提示文案

#### Scenario: 用户在轮询期间手动浏览后续结果
- **WHEN** 用户向下滚动结果列表且后续目录或正文批次更新进度与结果
- **THEN** SearchPalette MUST 保留用户的滚动位置
- **AND** 只有查询文本变化或用户显式键盘导航时 MAY 主动改变结果列表滚动位置
