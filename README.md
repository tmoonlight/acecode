...
For a project-local installation, run `npm install @aceagent/acecode` and launch
it with `npx acecode`. The optional desktop shell is available as
...
For the macOS desktop app, download the DMG matching the Mac architecture,
open it, and drag `ACECode.app` onto the `Applications` target. ACECode is
installed for the current user at `~/Applications/ACECode.app`. See
...
| `/remote-control` (alias `/rc`) | Activate a configured channel plugin or manage manual remote-control webhooks. See [Remote Control](#remote-control-channel-plugin). |
...
| `web_search` | Auto | Search RSS, DuckDuckGo, and Bing CN concurrently, then merge the results. |

By default, `web_search` sends each query (maximum 200 Unicode characters) concurrently to AceCode's hosted RSS index at `https://ge.bigjuan.xyz/rss-search`, DuckDuckGo, and Bing CN. Results are interleaved, deduplicated by URL, and capped by the requested total `limit`; if one source fails, successful sources are still returned with a warning. Set `web_search.backend` to `rss`, `auto`, `duckduckgo`, or `bing_cn` for explicit single/legacy behavior, or override `web_search.rss_base_url` for a self-hosted deployment. Custom endpoints require HTTPS; plaintext HTTP is accepted only for loopback addresses.

MCP servers can add more tools at runtime.

### Remote Control (Channel Plugin)

`/remote-control` (alias `/rc`) can hand the current TUI session to an external channel plugin: messages from the channel become user turns in this session, and the assistant's replies are pushed back through the plugin-provided outbound webhook. ACECode ships the generic channel protocol; the channel implementation itself lives outside this repository.

```
┌────────────────┐  POST /rc/send {"text"}    ┌──────────────────────────┐
│ channel plugin │ ─────────────────────────► │ ACECode TUI              │
│  or webhook    │   X-ACECode-RC-Token       │  loopback HTTP listener  │
...
1. Configure `remote_control.default_channel` and the matching `remote_control.channels.<name>.manifest_path` in `~/.acecode/config.json`.
2. `/rc` or `/remote-control` — activates the configured default channel plugin, generates or reuses the token, passes the session id and inbound credentials to the plugin, and installs the plugin's outbound webhook.
...
**Outbound — ACECode → channel.** At the end of each turn, every new assistant text block is POSTed to `outbound_url` as JSON:
...
| `web_search` | Web-search enablement, hosted RSS endpoint, and backend choice (`parallel` by default). |
