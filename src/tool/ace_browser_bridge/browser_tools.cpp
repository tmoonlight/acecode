...
std::string cli_prompt_for_session(const std::string& session) {
    ...
    oss
        ...
        << "`batch` runs steps in one browser dispatch, stops on the first failing step by default, and supports vars, `${...}` interpolation, `set`, `when`, `retry`, and top-level `finally`.\n\n"
        ...
        << "- For a sequence of related operations, prefer one `batch` with per-step `expect` / `assert`; for separate single commands, wrap browser work with `block-input` first and a final `unblock-input`.\n"
