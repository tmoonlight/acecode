...
std::string format_results_markdown(const std::string& query,
    ...
    for (const auto& h : resp.hits) {
        ...
        if (!h.source.empty() || !h.published_at.empty()) {
            oss << "\n   ";
            if (!h.source.empty()) oss << h.source;
            if (!h.source.empty() && !h.published_at.empty()) oss << " | ";
            if (!h.published_at.empty()) oss << h.published_at;
        }
        if (!snippet.empty()) {
            oss << "\n   " << snippet;
        }
