...
std::string build_system_prompt(const ToolExecutor& tools, const std::string& cwd,
    ...
    oss << "# Using your tools\n\n"
        ...
        << "- Before editing or overwriting an existing non-empty file, read the target file first. A non-lossy partial read establishes a baseline; stale files still require re-reading.\n"
        ...
        << "- If file_edit reports an encoding, read-baseline, or old_string failure, re-read the current content and retry with a corrected exact old_string instead of bypassing with shell, Python, or PowerShell writes.\n"
