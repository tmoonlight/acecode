...
//     写法,不能因文件名带点号被误降级。
//   - href 是远程 URL 但 host 解析失败(file:///、畸形)→ 无目标可比,放行。
...
// 从 URL / 裸域名中提取 host(去 scheme、userinfo、端口、路径/查询/片段,
// 去尾部句点,小写化)。本地路径、非 http(s) 协议(mailto:/file:/data: 等)、
// 含非 ASCII(IDN/百分号编码不做)解析失败 → nullopt。
