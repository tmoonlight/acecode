...
bool is_known_backend_name(const std::string& s) {
    return s == "rss" || s == "duckduckgo" || s == "bing_cn" ||
           s == "bochaai" || s == "tavily";
}
    ...
    if (sub.rfind("use", 0) == 0) {
        ...
        if (rest.empty()) {
            return "Usage: /websearch use <backend>\n"
                   "Available: rss, duckduckgo, bing_cn";
        }
        if (!is_known_backend_name(rest)) {
            return "Unknown backend '" + rest +
                   "'. Valid: rss, duckduckgo, bing_cn (bochaai/tavily not implemented yet)";
        }
        if (!is_implemented_backend_name(rest)) {
            return "Backend '" + rest +
                   "' is not implemented yet. Use rss, duckduckgo, or bing_cn.";
        }
...
void register_websearch_command(CommandRegistry& registry) {
    registry.register_command({
        "websearch",
        "Show or switch the web search backend (RSS / DuckDuckGo / Bing CN)",
        cmd_websearch,
    });
}
