...
inline ChatRenderWindow full_chat_render_window(
    ...
    out.total_rows = chat_transcript_display_rows(
        line_counts, out.last_message_exclusive);
    return out;
}
...
inline ChatRenderWindow chat_render_window(
    ...
    out.total_rows = chat_transcript_display_rows(line_counts, message_count);
    ...
    if (viewport_rows <= 0) {
        out = full_chat_render_window(line_counts, message_count);
        out.overscan_rows = std::max(0, overscan_rows);
        return out;
    }
    ...
    for (int i = 0; i < message_count; ++i) {
        ...
        const int message_rows = chat_line_count_at(line_counts, i) + 1;
    ...
    if (first < 0) {
        ...
        const int fallback_rows = chat_line_count_at(line_counts, first) + 1;
