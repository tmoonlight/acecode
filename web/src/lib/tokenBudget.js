...
// `promptTokens` is the total billed input on every provider (the Anthropic
...
export function normalizeTokenBudget({ usage = null, contextWindow = 0 } = {}) {
  ...
  const cacheTitle = cacheHitPercent === null
    ? ''
    : `提示缓存命中：${cacheReadTokens} / ${usedTokens} 输入 token（${cacheHitPercent}%），缓存写入 ${cacheWriteTokens}`;
