...
ace-browser-host.exe list-tabs --json --session acecode-demo --all
...
`read-page` 跨所有 frame 聚合元素。iframe(含跨域 iframe,如嵌入式支付字段)内的元素也会出现在 `elements` 里,并带 `frame_id` / `frame_url` 标注,顶层返回 `frame_count` 和 `iframe_element_count`。主 frame 元素的 `@e` 编号保持不变,iframe 元素接在其后统一编号。直接用返回的 `@e` ref 做 `click` / `fill` / `type`,插件会把动作按 ref 路由回对应 frame 执行。结构化 locator、CSS selector、`--target-text` 只在主 frame 解析,iframe 目标请优先用 `@e` ref。

点击预期会打开新窗口的场景(OAuth、PayPal 等支付弹窗、`target=_blank`)中,新窗口是独立 tab,session 不会自动跟随。处理方式:

```powershell
# 点击触发弹窗的按钮(@e 来自 read-page)
ace-browser-host.exe click --json --session acecode-demo --target @e12
# 枚举所有浏览器 tab,找到 opener_tab_id 等于当前 session tab 的新窗口
ace-browser-host.exe list-tabs --json --session acecode-demo --all
# 接管该弹窗 tab 继续操作
ace-browser-host.exe find-tab --json --session acecode-demo --tab-id <popup-tab-id>
ace-browser-host.exe read-page --json --session acecode-demo --mode summary
# 完成后切回原页面
ace-browser-host.exe find-tab --json --session acecode-demo --active
```

`list-tabs --all` 返回的每个 tab 带 `tabId`、`url`、`title`、`windowId`、`opener_tab_id` 和 `managed`(是否本扩展 session 管理)。`managed:false` 的项就是浏览器里其它 tab,弹窗通常出现在这里,可用 `opener_tab_id` 确认它是当前 session 点击触发的。
...
`batch` 从 stdin、`--stdin-input-json` 或 `--steps-file` 读取 JSON steps 数组或 `{steps, vars, finally}` 对象，并作为一个浏览器插件 action 顺序执行。默认首个失败步骤会停止后续步骤；单步可设置 `continue_on_error:true`。交互步骤会自动续期短时 AI 操作提示，失败路径也由页面侧 watchdog 自动释放。
...
浏览器交互动作会自动在受管页面上显示蓝绿色边框和 `AI 正在操作浏览器` 提示，并拦截页面内容区的鼠标、键盘、滚轮和触摸输入。每次操作都会把提示 watchdog 续期到默认 10 秒；调用方不需要执行单独的准备或清理命令。

提示只作用于页面内容区，不拦截浏览器地址栏、标签栏、扩展 UI 或系统级快捷键。动作异常或调用方中断时，content script 的 watchdog 会自动移除提示并释放输入。
