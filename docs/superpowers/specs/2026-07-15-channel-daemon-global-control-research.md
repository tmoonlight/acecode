...
| `/models`、`/model` | 列配置模型；切换当前 session 模型 | 可允许，但明确“下一轮生效” | owner/admin；禁止 `--global` |
...
| `/approve <request-id>` | 批准当前 session 的指定权限请求 | 立即 | owner/admin；必须带 request id，禁止 daemon 全局“批准最老请求” |
...
- `/pending`：列当前 route/session 的权限请求和问题，方便恢复断线期间的交互。
...
- 始终立即：`/help`、`/status`、`/whoami`、`/stop`、`/approve`、`/deny`、`/answer`；
- 始终可排队：普通文本、`/queue`；
