...
- Windows MCP 现在用结构化 `command + argv + env` 启动原生程序，含空格和 shell
  元字符的安装路径已有测试；现有 setup 注入的 `external-gateway --mcp --cwd <root>` 形状
...
同时，参考客户端 `master@264589a` 与 setup `master@09f947a` 已真正完成 SQLCipher 七天消息
...
- 若 gateway 后续随宿主原生包发布到 Linux/macOS，再补 POSIX plugin 生命周期、重启恢复和
