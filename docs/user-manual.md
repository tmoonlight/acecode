...
Desktop 的“添加上下文”菜单使用一个“添加图片、文件或文件夹”入口。它打开同一个系统文件窗口：正常选择一个或多个普通文件时只记录本机来源路径，不会读取、复制或 Base64 上传文件，因此大文件不受 25 MiB 快照限制；图片仍按原有方式生成会话快照并保留图片附件限制。在 Windows 窗口中进入目标目录并点击“选择当前文件夹”，会把地址栏当前目录插入原光标处。目录在 cwd 内时使用相对 `@folder/`，在 cwd 外或当前没有 cwd 时使用绝对路径引用（例如 `@D:/shared/`），不会递归上传目录内容。浏览器直连模式没有 Desktop 原生 bridge 时仍使用浏览器文件上传流程。

Desktop 文件树中也可以右键文件夹并选择“添加到会话”。该动作不强制要求 cwd，会在输入框当前光标处插入可用的相对或绝对 `@folder/` 引用，不会读取或上传文件夹内容；文件的同名右键动作仍按原有方式引用文件内容。

### Desktop 文件详情与标签页

在 Desktop 文件树打开工作区内的文本文件后，可以在详情页直接进入编辑：普通文本和代码使用轻量源码编辑框，Markdown 使用轻量所见即所得编辑器。两种编辑器都支持 `Ctrl+S` 保存；保存会保留原文件的编码、BOM 和换行风格。工作区外的绝对文件仍可预览，但不会提供编辑入口。

尚未保存的标签页显示实心大点。草稿会跟随标签页保留，切换标签或用详情面板右上角叉号隐藏面板都不会丢失；再次打开详情会恢复原标签、草稿和浏览器页面。只有在标签页上执行“关闭”“关闭其他”“关闭右侧”或“全部关闭”时才会移除标签，涉及未保存草稿时会先确认。文件在编辑期间被外部程序或 Agent 改动时，保存会提示冲突并保留当前草稿，不会静默覆盖磁盘内容。
