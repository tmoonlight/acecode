...
### 守护进程模式（HTTP / WebSocket 服务器）

ACECode 可以以后台守护进程方式运行，通过 HTTP 和 WebSocket 暴露 agent loop —
适合即将到来的 Web UI、CI 集成、或在无头服务器 SSH 远程使用。

三种启动方式任选其一：

```bash
# 1. 前台 — 日志输出到 stderr，阻塞当前终端（开发 / 调试）
./acecode daemon --foreground

# 2. 后台 — POSIX double-fork / Windows DETACHED_PROCESS 自动 detach
./acecode daemon start
./acecode daemon status        # {pid, port, guid, last_heartbeat_age_ms}
./acecode daemon stop          # 优雅 SIGTERM + 10 秒超时

# 3. Windows 服务 — 注册到 SCM，开机时早于登录框启动
#    （仅 Windows；install/uninstall 需要在管理员 PowerShell 执行）
...
**默认端点**：`http://127.0.0.1:28080`（配置项 `web.bind` / `web.port`）。
端口被占用时**直接拒启**，不会 retry 或换端口；请修改 `config.json` 的
`web.port` 或释放该端口。

**鉴权**：每次启动随机生成 32 字节 URL-safe base64 token，写入
`<数据目录>/run/token`（POSIX `0600` / Windows 仅当前用户 ACL）。loopback
客户端可省略 token；非 loopback 客户端必须在请求头 `X-ACECode-Token: <token>`
或 WebSocket 连接 URL `?token=<token>` 中携带。

**数据目录**：
- TUI / `acecode daemon ...` 模式：`~/.acecode/`
- `acecode service ...` 模式：`%PROGRAMDATA%\acecode\`（Windows）/
  `/Library/Application Support/acecode/`（macOS）/ `/var/lib/acecode/`（Linux）
- 两套根目录相互独立 — v1 中 service 模式 daemon 与 TUI **不共享** session /
  config / memory
...
完整 HTTP / WebSocket 协议参考：[`docs/daemon-api.md`](docs/daemon-api.md)。

#### Linux：systemd unit 示例

`acecode service install` 仅 Windows。Linux 用 systemd 跑守护进程，保存为
`/etc/systemd/system/acecode.service`：
...
Restart=on-failure
RestartSec=5
# 以你的普通用户身份运行；daemon 会使用该用户的 ~/.acecode
User=youruser
...
启用：`sudo systemctl daemon-reload && sudo systemctl enable --now acecode`。

#### macOS：launchd plist 示例

保存为 `~/Library/LaunchAgents/dev.acecode.daemon.plist`：
...
加载：`launchctl load ~/Library/LaunchAgents/dev.acecode.daemon.plist`。
